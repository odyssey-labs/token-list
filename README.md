...
Each account on NEAR can have at most one contract deployed to it. If you've already created an account such as `your-name.testnet`, you can deploy your contract to `token-list.your-name.testnet`. Assuming you've already created an account on [NEAR Wallet], here's how to create `token-list.your-name.testnet`:
...
2. Create a subaccount (replace `YOUR-NAME` below with your actual account name):
      ...
      near create-account token-list.YOUR-NAME.testnet --masterAccount YOUR-NAME.testnet
...
Modify the line in `src/config.js` that sets the account name of the contract. Set it to the account id you used above.
    ...
    const CONTRACT_NAME = process.env.CONTRACT_NAME || 'token-list.YOUR-NAME.testnet'
